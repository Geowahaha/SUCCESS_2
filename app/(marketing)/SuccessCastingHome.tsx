...
function HeroSlider() {
  ...
  return (
    <section
      className="relative min-h-[calc(100vh-92px)] overflow-hidden bg-zinc-950 text-white sm:min-h-[760px]"
      aria-label="Success Casting material image hero slideshow"
    ...
    >
      ...
      <button type="button" aria-label="Next background product" onClick={(event) => { event.stopPropagation(); goBg(1); }} className="absolute right-4 top-1/2 z-30 grid h-12 w-12 -translate-y-1/2 place-items-center rounded-full border border-white/25 bg-black/45 text-3xl font-black text-white backdrop-blur transition hover:bg-[#d99d2d] hover:text-zinc-950 sm:right-6 lg:right-24">
        ›
      </button>

      <div className={`absolute right-4 top-1/2 z-20 hidden -translate-y-1/2 overflow-hidden rounded-3xl bg-[#d99d2d] p-2 shadow-2xl transition-opacity duration-500 lg:block ${focusImage ? "opacity-0 pointer-events-none" : "opacity-100"}`}>
      ...
      <Link href="/contact" onClick={(event) => event.stopPropagation()} className={`absolute right-4 top-6 z-30 hidden rounded-full bg-[#d99d2d] px-6 py-4 text-center text-sm font-black uppercase tracking-[0.08em] text-zinc-950 shadow-2xl shadow-black/30 transition hover:scale-105 hover:bg-white sm:inline-flex lg:right-8 ${focusImage ? "opacity-0 pointer-events-none" : "opacity-100"}`}>
        Request a Quote
      </Link>

      <button type="button" onClick={(event) => { event.stopPropagation(); setFocusImage((value) => !value); setPaused(true); }} className="absolute bottom-5 right-5 z-30 rounded-full border border-white/20 bg-black/45 px-4 py-2 text-xs font-bold text-white/90 backdrop-blur transition hover:bg-white hover:text-zinc-950">
      ...
      <div className="pointer-events-none absolute bottom-5 left-1/2 z-30 flex -translate-x-1/2 gap-2">
        {materials.map((item, index) => (
          <button
            ...
            className={`pointer-events-auto h-2.5 rounded-full transition-all ${index === activeBg ? "w-8 bg-[#d99d2d]" : "w-2.5 bg-white/50 hover:bg-white"}`}
          />
        ))}
      </div>
      ...
      <div className="relative mx-auto flex min-h-[calc(100vh-92px)] max-w-7xl flex-col justify-center px-5 py-20 sm:min-h-[760px] sm:px-6 lg:px-8">
        <div className={`max-w-3xl transition-all duration-500 ${focusImage ? "pointer-events-none translate-y-4 opacity-0" : "translate-y-0 opacity-100"}`}>
          <div className="inline-flex rounded-full bg-black/45 px-4 py-2 text-[11px] font-black uppercase tracking-[0.22em] text-zinc-100 backdrop-blur sm:text-xs">
            Professional Sand Casting
          </div>
          <p className="mt-5 inline-flex rounded-full bg-[#d99d2d] px-4 py-2 text-[11px] font-black uppercase tracking-[0.20em] text-zinc-950 shadow-xl shadow-black/30 sm:px-5 sm:py-3 sm:text-xs">FC25 · FCD · SC46 · S45C · Mo4140</p>
          ...
          <p className="mt-6 max-w-2xl border-l-4 border-[#d99d2d] pl-5 text-base leading-8 text-zinc-100 drop-shadow-lg sm:text-lg">
          ...
          <div className="mt-8 flex flex-col gap-3 sm:flex-row">
            <Link href="/contact" onClick={(event) => event.stopPropagation()} className="inline-flex justify-center rounded-2xl bg-[#d99d2d] px-8 py-4 text-sm font-black uppercase tracking-wide text-zinc-950 shadow-xl shadow-black/30 hover:bg-white sm:text-base">
              Request a Quote
            </Link>
            <Link href="/products" onClick={(event) => event.stopPropagation()} className="inline-flex justify-center rounded-2xl border border-white/35 bg-black/25 px-8 py-4 text-sm font-black text-white backdrop-blur hover:bg-white/10 sm:text-base">
              ดูผลงานสินค้า
            </Link>
          </div>
...
function TopBar() {
  return (
    <>
      <div className="bg-black px-4 py-2.5 text-xs text-white sm:text-sm">
      ...
      <header className="sticky top-0 z-40 border-b border-white/10 bg-[#080808]/92 text-white shadow-lg backdrop-blur">
        <div className="mx-auto flex max-w-7xl flex-col gap-3 px-4 py-3 md:flex-row md:items-center md:justify-between">
          <Link href="/" className="flex items-center gap-3">
            <img src={pulleyLogo} alt="Success Network Company logo" className="h-20 w-20 object-contain sm:h-24 sm:w-24" decoding="async" />
            <span>
          ...
          <nav className="flex flex-wrap gap-2 text-sm font-semibold md:justify-end">
            <Link className="rounded px-3 py-2 hover:bg-[#d99d2d] hover:text-zinc-950" href="/">หน้าแรก</Link>
            <Link className="rounded px-3 py-2 hover:bg-[#d99d2d] hover:text-zinc-950" href="/products">สินค้า</Link>
            <a className="rounded px-3 py-2 hover:bg-[#d99d2d] hover:text-zinc-950" href="#materials">วัสดุที่รับผลิต</a>
            <a className="rounded px-3 py-2 hover:bg-[#d99d2d] hover:text-zinc-950" href="#why">ทำไมต้องเรา</a>
            <a className="rounded px-3 py-2 hover:bg-[#d99d2d] hover:text-zinc-950" href="#contact">ติดต่อเรา</a>
            <Link className="rounded-2xl bg-[#d99d2d] px-4 py-2 font-black text-zinc-950 hover:bg-white" href="/contact">ขอใบเสนอราคา</Link>
          </nav>
        </div>
      </header>
    </>
...
function MaterialSlider() {
  ...
  return (
    <section id="materials" className="bg-[#f4f1ea] px-4 py-16 text-zinc-800 sm:px-6 lg:px-8" onPointerDown={() => setPaused(true)} onTouchStart={() => setPaused(true)}>
      <div className="mx-auto grid max-w-7xl gap-8 lg:grid-cols-[.82fr_1.18fr] lg:items-center">
        <div>
          <p className="inline-flex rounded-full bg-[#d99d2d] px-4 py-2 text-xs font-black uppercase tracking-[0.20em] text-zinc-950">วัสดุหลักที่รับผลิต</p>
          ...
          <p className="mt-5 text-base leading-8 text-zinc-600 sm:text-lg">
            Success Casting โฟกัสงานหล่อทรายและชิ้นส่วนเครื่องจักรตามแบบลูกค้า พร้อมให้คำแนะนำวัสดุตามลักษณะการใช้งานจริง
          </p>
        ...
        <div className="overflow-hidden rounded-[1.5rem] border border-zinc-200 bg-white shadow-2xl shadow-black/15">
          <div className="relative min-h-[420px]">
            ...
            <div className="absolute inset-x-0 bottom-0 p-6 sm:p-8">
              <div className="mb-4 inline-flex rounded bg-[#d99d2d] px-4 py-2 text-2xl font-black text-zinc-950">{current.code}</div>
    ...
    <section id="contact" className="bg-[#222] px-4 py-12 text-white sm:px-6 lg:px-8">
      <div className="mx-auto grid max-w-7xl gap-6 lg:grid-cols-[.9fr_.78fr_1.05fr] lg:items-stretch">
        <div className="rounded-3xl bg-white/5 p-6 ring-1 ring-white/10">
          <p className="text-xs font-black uppercase tracking-[0.24em] text-[#d99d2d]">ติดต่อเรา</p>
          ...
          <div className="mt-5 space-y-2 text-sm leading-7 text-zinc-200 sm:text-base">
            <p><span className="font-bold text-white">บริษัท:</span> บริษัท ซัคเซสเน็ทเวิร์ค จำกัด</p>
            <p><span className="font-bold text-white">ที่อยู่:</span> 307/288 หมู่ที่ 11 ต.บางพลีใหญ่ อ.บางพลี จ.สมุทรปราการ 10540</p>
            <p><span className="font-bold text-white">โทร:</span> <a href="tel:0986362356" className="text-[#d99d2d] hover:text-white">098-636-2356</a></p>
            <p><span className="font-bold text-white">Email:</span> <a href="mailto:SCNWMax@gmail.com" className="text-[#d99d2d] hover:text-white">SCNWMax@gmail.com</a></p>
            <p><span className="font-bold text-white">LINE:</span> <a href={lineUrl} className="text-[#d99d2d] hover:text-white">@SCNW</a></p>
          </div>
          <div className="mt-6 flex flex-col gap-3 sm:flex-row">
            <a href={lineUrl} className="rounded-2xl bg-[#d99d2d] px-6 py-3 text-center font-black text-zinc-950 hover:bg-white">ส่งรูปผ่าน LINE</a>
        ...
        <a href={lineUrl} className="flex items-center justify-center gap-5 rounded-3xl bg-white p-5 text-zinc-950 shadow-2xl shadow-black/30 lg:flex-col">
          ...
          <div className="text-left lg:text-center">
            <div className="text-lg font-black">LINE ID: @SCNW</div>
    ...
    <main className="bg-white text-zinc-800">
      ...
      <section className="bg-white px-4 py-16 sm:px-6 lg:px-8">
        <div className="mx-auto grid max-w-7xl gap-8 lg:grid-cols-[.75fr_1fr] lg:items-center">
          <div>
          <p className="mb-4 inline-flex rounded-full bg-[#d99d2d] px-4 py-2 text-xs font-black text-zinc-950">About Us</p>
          ...
          <div className="relative overflow-hidden rounded-[1.5rem] bg-[#d99d2d] p-3 shadow-2xl shadow-black/15">
            <img src="/successcasting-assets/shopee-new/mold-boxes-fc25.jpg" alt="Success Casting mold boxes and casting portfolio" className="h-[420px] w-full rounded-[1.1rem] object-cover" decoding="async" />
            <Link href="/products" aria-label="ดูผลงานสินค้า 20+ รูป" className="group absolute bottom-7 left-7 rounded-2xl bg-black/80 px-5 py-4 text-white backdrop-blur transition hover:-translate-y-1 hover:bg-[#d99d2d] hover:text-zinc-950 focus:outline-none focus:ring-4 focus:ring-[#d99d2d]/60">
      ...
      <section className="bg-white px-4 pb-16 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-7xl">
          <h2 className="inline-block border-b-4 border-[#d99d2d] pb-2 text-4xl font-black uppercase tracking-[-0.04em] text-zinc-950 sm:text-5xl">ผลงานสินค้า</h2>
          ...
          <Link href="/products" className="mt-7 inline-flex rounded-2xl bg-[#d99d2d] px-7 py-3 font-black text-zinc-950 hover:bg-zinc-950 hover:text-white">ดูสินค้าทั้งหมด</Link>
        </div>
      </section>
      ...
      <section id="why" className="bg-white px-4 py-14 sm:px-6 lg:px-8">
        <div className="mx-auto grid max-w-6xl gap-6 md:grid-cols-3">
          {[
            ...
            <div key={title} className="rounded-2xl border border-zinc-200 bg-[#faf9f7] p-6">
